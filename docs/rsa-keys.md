---
sidebar_position: 3
---

# Using RSA Private/Public Keys

Instead of using a shared secret, you can use RSA private/public key pairs for enhanced security.

## Example

```php
<?php
$secret = <<<PRIVATE
-----BEGIN RSA PRIVATE KEY-----

-----END RSA PRIVATE KEY-----
PRIVATE;

$public = <<<PUBLIC
-----BEGIN PUBLIC KEY-----

owIDAQAB
-----END PUBLIC KEY-----
PUBLIC;

$sessionConfig = (new \ByJG\Session\SessionConfig('example.com'))
    ->withRsaSecret($secret, $public)
    ->replaceSessionHandler();

$handler = new \ByJG\Session\JwtSession($sessionConfig);
```

## Generating RSA Keys

If you want to know more details about how to create RSA Public/Private Keys, visit:
https://github.com/byjg/jwt-wrapper

## Benefits of RSA Keys

- **Enhanced Security**: Private key never needs to be shared with clients
- **Better Key Management**: Public key can be distributed safely for verification
- **Stronger Cryptography**: RSA provides asymmetric encryption capabilities
